...
//	建立變數
	...
	boolean ReversiSomething = false; //判斷是否有棋子可以翻轉 0: 沒有 1:有
	boolean GameOver = true; //判斷遊戲是否結束(死棋專用) 0:繼續 1:遊戲結束---------------------------
	boolean GameOver2 = true;  //判斷遊戲是否結束(放滿專用) 0:繼續 1:遊戲結束---------------------------
	boolean Reversi = true; //翻轉功能啟動 。0:關閉 。1:啟動
//	boolean openJudgeCheckmate = false; //開啟死期檢測功能 。0:關閉 。1；開啟
//	boolean Link = true;
//	String Winner = "";
...
//		for(int j = 0;j<2;j++) {
//			for(int i = 0;i<height;i++) {
//				infCheckerboard[j][i] = 1;
//			}
//		}
//		
//		for(int i = 0;i<height;i++) {
//			for(int j = 0;j<2;j++) {
//				infCheckerboard[i][j] = 1;
//			}
//		}
...
//	取的翻轉旗標
	public boolean getRST() {
		return ReversiSomething;
	}
//	重製翻轉旗標
	public void initRST() {
		ReversiSomething = false;
	}
//	取的結束旗標
...
//	判斷遊戲是否結束
	public void judgeGameOver() {
		...
		int tempQuantityWhite = 0;
		int tempQuantityBlack = 0;

		for(int i = 0;i<height;i++) {
			for(int j = 0;j<width;j++) {
//				計算棋數
				if(infCheckerboard[i][j] == 1) {
					tempQuantityWhite++;
				}
//				計算棋數
				if(infCheckerboard[i][j] == 2) {
					tempQuantityBlack++;
//					GameOver = false;
				}
//				棋盤上沒有空位
		...
		quantityWhite = tempQuantityWhite;
		quantityBlack = tempQuantityBlack;
	}
//	取得贏家
...
//	}
//	取的目前攻擊方棋數
	public int getQuantityWhite() {
		return quantityWhite;
	}
//	取的目前防守方棋數
	public int getQuantityBlack() {
		return quantityBlack;
	}
//	初始化棋數量
...
//		openJudgeCheckmate = true;
//		initGameOver();
		int value = 0;
//		System.out.println("judgeCheckmate: ON");
		Reversi = false;
		for(int i = 0;i<height;i++) {
			for(int j = 0;j<width;j++) {
				value++;
//				System.out.print("Round: " + (value-1) + "\t");
				if(infCheckerboard[i][j] != 0) {
					System.out.println("Location: " + (value-1) + "\t" + j + " " + i);
					continue;
				}else {
					System.out.println("Location: " + (value-1) + "\t" + j + " " + i);
				}
